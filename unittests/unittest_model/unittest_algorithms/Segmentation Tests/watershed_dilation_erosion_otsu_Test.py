...
import numpy as np
from watershed_dilation_erosion_otsu import *
from ImageSegmentationCollection import *
import cv2
import unittest

class TestWatershedDilationErosionOtsu(unittest.TestCase):

  def test_instantiation(self):
    obj = AlgBody()
    self.assertEqual(len(obj.integer_sliders), 2)
    self.assertEqual(obj.name, "Watershed - Dilation Erosion Otsu")
  ...
  def test_segmentation(self):
    ...
    ref_image1 = guided_watershed_deletion_erosion_otsus_threshold(src=test_image,
                                                                   fg_iter=2,
                                                                   bg_iter=1)
    ref_image2 = guided_watershed_deletion_erosion_otsus_threshold(src=test_image,
                                                                   fg_iter=5,
                                                                   bg_iter=4)
