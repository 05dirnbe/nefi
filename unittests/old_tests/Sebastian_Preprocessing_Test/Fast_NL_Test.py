...
import numpy as np
from Sebastian_Algorithms_untested.fast_nl_denoise import *
import cv2
import unittest
...
class TestNLMeans(unittest.TestCase):
  ...
  def test_instantiation(self):
    ...
    self.assertEqual(obj.name, "Fast nl Means Denoising")
    self.assertEqual(obj.parent, "Preprocessing")
    self.assertEqual(obj.float_sliders[0].name, "filter strength")
    ...
    self.assertEqual(obj.integer_sliders[0].name, "template window size")
    ...
    self.assertEqual(obj.integer_sliders[0].value, 3)

    self.assertEqual(obj.integer_sliders[1].name, "search window size")
    self.assertEqual(obj.integer_sliders[1].lower, 1)
    self.assertEqual(obj.integer_sliders[1].upper, 20)
  ...
  def test_process_whole_channels(self):
    ...
    obj2.integer_sliders[0].set_value(9)
    ...
    for i in range(h):
        for j in range(w):
            for k in range(d):
                ...
                self.assertLessEqual(diff1,25)
                self.assertLessEqual(diff2,25)
