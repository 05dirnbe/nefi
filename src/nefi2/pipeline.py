...
import os
import sys
sys.path.insert(0, os.path.join(os.curdir, 'modules', 'methods'))
sys.path.insert(0, os.path.join(os.curdir, 'modules', 'algorithms'))
...
class ModScanner:
    """
    A class that scans for new files in /modules/algorithms and
    /modules/methods. It checks these files for compliance with the interface,
    ...
    """
    def __init__(self):
        self.algdir = os.path.join('modules', 'algorithms')
        self.methdir = os.path.join('modules', 'methods')
        self.scan_dirs()


    def scan_dirs(self):
        """
        Search for new files in modules directory and check their
...
class Pipeline:
    ...
    def run_container_method(self):
        ...
        print '> Pipeline: Running "%s" on "%s"' % (self.method, self.image)
