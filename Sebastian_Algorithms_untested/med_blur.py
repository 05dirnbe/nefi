...
class AlgBody(Algorithm):
    ...
    def process(self, image):
        ...
        if self.channel1:
            self.channels[0] =cv2.medianBlur(self.channels[0],self.kernelsize.value*2+1)
        if self.channel2:
            self.channels[1] =cv2.medianBlur(self.channels[1],self.kernelsize.value*2+1)
        if self.channel3:
            self.channels[2] =cv2.medianBlur(self.channels[2],self.kernelsize.value*2+1)
